...
function loadNavbar(activePage) {
  ...
  navbar.innerHTML = `
    ...
    <div class="nav-links" id="navLinks">
      ${navLinks}
      <button class="lang-toggle btn-ghost btn-sm" onclick="openLanguagePicker()" style="display:flex;align-items:center;gap:4px;padding:6px 10px;border:none;background:var(--primary-light);border-radius:var(--radius-sm);cursor:pointer;color:var(--primary);font-size:12px;font-weight:600;position:relative">
      ...
      ${isLoggedIn ? `<a href="/profile.html" class="btn btn-ghost btn-sm" data-i18n="nav.profile">${t('nav.profile')}</a><button class="btn btn-ghost btn-sm" onclick="logout()" data-i18n="nav.logout">${t('nav.logout')}</button>` : `<a href="/login.html" class="btn btn-primary btn-sm" data-i18n="nav.signIn">${t('nav.signIn')}</a>`}
    </div>
  `;
  initTheme();
}
